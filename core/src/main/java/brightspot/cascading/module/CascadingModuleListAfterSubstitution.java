...
public class CascadingModuleListAfterSubstitution extends CascadingModuleListAfter implements GroupedPlaceItem,
        Substitution {

    @Override
    public List<Place> getGroupedPlaceItemPlaces(Placeable source, Recordable target) {
        return Collections.singletonList(new GroupedPlace(source, target, "After", getItems()));
    }
}
